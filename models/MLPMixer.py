...
class ResNet(nn.Module):
    def __init__(self, hidden_sizes, num_blocks, input_dim, in_channels, tgt_vocab=957, fc_num_layers=0, 
        ...
        if self.use_mixer:
            ...
            self.head = nn.Linear(token_dims, tgt_vocab)
        elif fc_num_layers:
            self.fc = self._create_mlp_block(fc_init_dim=num_tokens*token_dims, fc_dim=fc_dim, fc_num_layers=fc_num_layers)
            self.fc_head = nn.Linear(fc_dim, tgt_vocab)

        else:
            self.linear = nn.Linear(num_tokens*token_dims, self.tgt_vocab)
